...
If you want to start a fresh project using `now init` refer to <GenericLink href="#deploying-a-new-cra-project-using-the-now-cli">Deploying using now init</GenericLink>.
...
## Deploying a new CRA project using the Now CLI

### Pre-requisites:

The method of initializing a project with Now requires Now CLI version 13.0.0 or above.

If you need to install Now CLI and haven't followed the guide above, you can install Now CLI via [Now Desktop](/docs/v2/getting-started/installation/#now-desktop) for automatic updates or [directly](/docs/v2/getting-started/installation/#now-cli).
...
<TerminalOutput>
$ now -v <br/>
14.0.2</TerminalOutput>
<Caption>`now -v` will show you your currently installed Now version.</Caption>

The easiest way to create and deploy a new project with CRA is to use the templates provided by the now cli.
...
<Caption>`now init` will give you a list of projects you can choose, for this guide we're using the `create-react-app` example</Caption>

This will make a new directory called `create-react-app` with everything configured out of the box.
